...
<script>
    ...
    export default {
		...
		props: {
			...
			statusFilterIndex: { type: Number, default: -1 },
		},
		data() {
			return {
				defaultOptions: { 
        ...
        methods: {
            callback() {
				...
				window.datatables = new DataTables(options, this.print);
				window.datatables.onLoaded = this.setClickEvents;

				this.loadFilters();

				if (this.statusFilterIndex > -1) {
					this.loadStatusFilter(this.statusFilterIndex);
				}
            },
			loadFilters() {
